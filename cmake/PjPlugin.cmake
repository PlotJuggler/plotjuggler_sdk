...
#
# Mach-O has no STB_GNU_UNIQUE, but it has its own sharing channel: every weak
# C++ symbol a plugin dylib EXPORTS joins dyld's process-wide weak coalescing
# at dlopen, so plugins can end up sharing one copy's statics — and part of a
# plugin's own references can be rebound across images, which breaks
# pointer-based identity (typeinfo, inline statics) inside the plugin. The
# Darwin branch therefore restricts exports with ld64's -exported_symbols_list.
# ld64 treats a literal entry as REQUIRED (undefined-symbol error when absent),
# unlike a version script, so the family getters are one glob — an unmatched
# glob is silently ignored.
function(pj_harden_plugin_exports TARGET)
  ...
  if(CMAKE_SYSTEM_NAME STREQUAL "Darwin")
    set(_exports
      _pj_plugin_abi_version
      "_PJ_get_*_vtable"
      "_pj_plugin_descriptor_*")
    foreach(_symbol IN LISTS ARG_REQUIRED_EXPORTS ARG_EXTRA_EXPORTS)
      # EXTRA/REQUIRED entries keep version-script semantics (absent = no-op),
      # so they are emitted as prefix globs rather than required literals.
      list(APPEND _exports "_${_symbol}*")
    endforeach()
    list(REMOVE_DUPLICATES _exports)
    list(JOIN _exports "\n" _export_lines)
    set(_list "${CMAKE_CURRENT_BINARY_DIR}/${TARGET}_exports.exp")
    file(CONFIGURE OUTPUT "${_list}" CONTENT
