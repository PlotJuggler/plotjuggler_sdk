...
```cpp
// pj_datastore/include/pj_datastore/derived_engine.hpp
...
| File | Change |
|---|---|
| `pj_base/include/pj_base/types.hpp` | Add `NodeId = uint32_t`, `kInvalidNodeId = 0` |
| `CMakeLists.txt` | Add `derived_engine.cpp`, `builtin_transforms.cpp` to `pj_datastore`; add `derived_engine_test` target |
