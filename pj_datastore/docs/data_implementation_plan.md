...
> two libraries: `pj_base` (types, type tree, dataset metadata --
> zero dependencies) and `pj_datastore` (everything else -- depends on
...
2. ✅ **engine-storage**: Owns typed column buffers and chunk lifecycle.
   ...
   *Implemented as `RawBuffer`, `TypedColumnBuffer`, `TopicChunkBuilder`,
   `TopicChunk`, `TopicStorage` in the `pj_datastore` library.*
3. ✅ **engine-derived**: Owns transform DAG, scheduler, dirty propagation.
   Depends on engine-core interfaces only.
   *Implemented as `DerivedEngine` in the `pj_datastore` library.
...
`ChunkStats` (`pj_datastore/chunk.hpp`). They are tracked incrementally during
...
`pj_datastore/derived_engine.hpp`:
...
Built-in transforms: `DerivativeTransform` (in `pj_datastore/builtin_transforms.hpp`).
...
|---|---|---|
| `pj_base` | `pj_base/` | None (zero deps) |
| `pj_datastore` | `pj_datastore/` | Abseil (absl::status, absl::flat_hash_map, ...), nanoarrow + nanoarrow_ipc (PRIVATE) |

### Headers

| File | Plan Concept |
|---|---|
| `base/include/pj_base/types.hpp` | Section 11: ID types, Timestamp, NumericType/NumericValue |
| `base/include/pj_base/type_tree.hpp` | Section 4.2: TypeTreeNode, PrimitiveType, EnumMapping |
| `base/include/pj_base/dataset.hpp` | Section 3/8: DatasetDescriptor, DatasetInfo, TimeDomain |
...
**`ColumnData` struct** (`pj_datastore/writer.hpp`): Describes one column's
...
1. **Engine uses nanoarrow, not full Arrow C++**: nanoarrow (a
   ...
   `pj_datastore` for the import adapter only. The bulk API
