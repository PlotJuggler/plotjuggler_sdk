...
This document describes the architecture of the software currently implemented in this repository, focused on the `data/` engine.
...
- `plotjuggler_base` in `data/base/`
- `plotjuggler_engine` in `data/engine/`
...
Primary build file: `data/CMakeLists.txt`

Main targets:

- `plotjuggler_base`
- `plotjuggler_engine`
- test executables in `data/tests/`
- benchmark executables in `data/benchmarks/`

Key dependencies from `data/conanfile.txt`:
...
File: `data/engine/src/chunk.cpp`.
...
File: `data/engine/src/topic_storage.cpp`.
...
- `data/engine/src/reader.cpp`
- `data/engine/src/query.cpp`
...
File: `data/engine/src/derived_engine.cpp`.
...
File: `data/engine/src/arrow_import.cpp`.
...
Test suite location: `data/tests/`.
...
Benchmark suite location: `data/benchmarks/`.
...
7. Run tests and benchmarks from `data/build/cmake_build`.
