...
void TopicChunkBuilder::begin_row(Timestamp timestamp) {
  ...
  PJ_ASSERT(timestamp >= last_timestamp_,
...
void TopicChunkBuilder::append_timestamps(const Timestamp* timestamps,
                                          std::size_t count) {
  PJ_ASSERT(!row_in_progress_,
            "append_timestamps called while row in progress");
  if (count == 0) return;

  PJ_ASSERT(timestamps[0] >= last_timestamp_,
            "timestamps must be monotonically non-decreasing");

  timestamps_.reserve(timestamps_.size() + count);
  timestamps_.insert(timestamps_.end(), timestamps, timestamps + count);
...
void TopicChunkBuilder::append_column_float32(std::size_t col_index,
                                              const float* data,
                                              std::size_t count) {
  PJ_ASSERT(col_index < columns_.size(), "col_index out of bounds");
  columns_[col_index].append_float32_bulk(data, count);
}

void TopicChunkBuilder::append_column_float64(std::size_t col_index,
                                              const double* data,
                                              std::size_t count) {
  PJ_ASSERT(col_index < columns_.size(), "col_index out of bounds");
  columns_[col_index].append_float64_bulk(data, count);
}

void TopicChunkBuilder::append_column_int32(std::size_t col_index,
                                            const int32_t* data,
                                            std::size_t count) {
  PJ_ASSERT(col_index < columns_.size(), "col_index out of bounds");
  columns_[col_index].append_int32_bulk(data, count);
}

void TopicChunkBuilder::append_column_int64(std::size_t col_index,
                                            const int64_t* data,
                                            std::size_t count) {
  PJ_ASSERT(col_index < columns_.size(), "col_index out of bounds");
  columns_[col_index].append_int64_bulk(data, count);
}

void TopicChunkBuilder::append_column_uint64(std::size_t col_index,
                                             const uint64_t* data,
                                             std::size_t count) {
  PJ_ASSERT(col_index < columns_.size(), "col_index out of bounds");
  columns_[col_index].append_uint64_bulk(data, count);
}

void TopicChunkBuilder::append_column_bool(std::size_t col_index,
                                           const uint8_t* data,
                                           std::size_t count) {
  PJ_ASSERT(col_index < columns_.size(), "col_index out of bounds");
  columns_[col_index].append_bool_bulk(data, count);
}

void TopicChunkBuilder::append_column_strings(std::size_t col_index,
                                              const uint32_t* offsets,
                                              const char* data,
                                              std::size_t count) {
  PJ_ASSERT(col_index < columns_.size(), "col_index out of bounds");
  columns_[col_index].append_strings_bulk(offsets, data, count);
}

void TopicChunkBuilder::append_column_validity(std::size_t col_index,
                                               const uint8_t* bitmap,
                                               std::size_t bit_offset,
                                               std::size_t count) {
  PJ_ASSERT(col_index < columns_.size(), "col_index out of bounds");
  columns_[col_index].append_validity_bulk(bitmap, bit_offset, count);
...
void TopicChunkBuilder::finish_bulk_append() {
  PJ_ASSERT(!row_in_progress_,
            "finish_bulk_append called while row in progress");
  if (bulk_pending_rows_ == 0) return;
...
void TopicChunkBuilder::compute_bulk_numeric_stats(
    std::size_t col_index, StorageKind kind,
  ...
  auto process = [&]<typename T>(const T* /*tag*/) {
    ...
    double local_min = cs.min_value.value_or(
        std::numeric_limits<double>::max());
    double local_max = cs.max_value.value_or(
...
void TopicChunkBuilder::compute_bulk_string_stats(std::size_t col_index,
      ...
      case StorageKind::kBool: {
        if (cs.is_constant && col.row_count() > 0) {
          ...
          chunk.encoding_data[i] = encoding::constant_encode(
              col.value_buffer().data(), kind, col.row_count());
        } else {
          chunk.column_encodings[i] = EncodingType::kPackedBool;
          chunk.encoding_data[i] = encoding::pack_bools(
...
double TopicChunk::read_numeric_as_double(std::size_t col_index,
  ...
  switch (column_encodings[col_index]) {
    case EncodingType::kConstant:
      return encoding::constant_decode_as_double(
          std::get<encoding::ConstantEncoded>(encoding_data[col_index]));
    case EncodingType::kFrameOfReference:
      return encoding::for_decode_one_as_double(
          std::get<encoding::FrameOfReferenceEncoded>(encoding_data[col_index]),
          row);
    case EncodingType::kRaw: {
      const StorageKind kind = storage_kind_of(
          column_descriptors[col_index].logical_type);
...
std::string_view TopicChunk::read_string(std::size_t col_index,
                                         std::size_t row) const {
  return encoding::dictionary_lookup(
      std::get<encoding::DictionaryEncoded>(encoding_data[col_index]), row);
}
...
bool TopicChunk::read_bool(std::size_t col_index, std::size_t row) const {
  ...
  return encoding::unpack_bool(
      std::get<encoding::PackedBools>(encoding_data[col_index]), row);
}
...
void TopicChunk::read_column_as_doubles(std::size_t col_index, double* out,
                                        std::size_t row_start,
                                        std::size_t count) const {
  switch (column_encodings[col_index]) {
    case EncodingType::kConstant: {
      const double val = encoding::constant_decode_as_double(
          std::get<encoding::ConstantEncoded>(encoding_data[col_index]));
      std::fill(out, out + count, val);
      return;
    }
    case EncodingType::kFrameOfReference: {
      encoding::for_decode_range_as_doubles(
          std::get<encoding::FrameOfReferenceEncoded>(
              encoding_data[col_index]),
          out, row_start, count);
      return;
    }
  ...
  switch (kind) {
    case StorageKind::kFloat32: return convert(static_cast<const float*>(nullptr));
    case StorageKind::kFloat64: return convert(static_cast<const double*>(nullptr));
    case StorageKind::kInt32:   return convert(static_cast<const int32_t*>(nullptr));
    case StorageKind::kInt64:   return convert(static_cast<const int64_t*>(nullptr));
    case StorageKind::kUint64:  return convert(static_cast<const uint64_t*>(nullptr));
    case StorageKind::kBool:
    case StorageKind::kString: {
      const double nan = std::numeric_limits<double>::quiet_NaN();
      for (std::size_t i = 0; i < count; ++i) {
