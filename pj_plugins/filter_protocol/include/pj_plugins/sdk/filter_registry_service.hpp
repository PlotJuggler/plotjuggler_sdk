...
class FilterRegistryView {
 public:
  ...
  [[nodiscard]] bool valid() const noexcept {
    return raw_.ctx != nullptr && raw_.vtable != nullptr &&
           raw_.vtable->register_transform != nullptr && raw_.vtable->create_transform != nullptr;
  }
  ...
  [[nodiscard]] Expected<void> registerTransform(std::string id, std::shared_ptr<void> library_owner) {
    ...
    auto* deleter = +[](PJ_filter_transform_t* p) noexcept {
      delete reinterpret_cast<FilterTransform*>(p);
    };
  ...
  [[nodiscard]] std::shared_ptr<FilterTransform> create(std::string_view id) const {
    ...
    return std::shared_ptr<FilterTransform>(
        reinterpret_cast<FilterTransform*>(raw), [deleter](FilterTransform* p) noexcept {
          deleter(reinterpret_cast<PJ_filter_transform_t*>(p));
        });
  }
 ...
 private:
  [[nodiscard]] Expected<void> registerRaw(
      std::string id, PJ_filter_transform_factory_fn factory, PJ_filter_transform_deleter_fn deleter,
      void* factory_ctx, std::shared_ptr<void> library_owner) {
