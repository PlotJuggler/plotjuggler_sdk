...
class FilterRegistryHost {
 ...
 private:
  static bool thunkRegister(
      void* ctx, PJ_filter_transform_registration_t reg, void* library_owner_token,
      PJ_error_t* out_error) noexcept {
    auto& self = selfOf(ctx);
    if (reg.factory == nullptr || reg.deleter == nullptr || reg.id.data == nullptr ||
        reg.id.size == 0) {
    ...
    sdk::FilterTransformFactory::DeleterFn delete_wrapper =
        [deleter_fn](sdk::FilterTransform* p) noexcept {
  ...
  static PJ_filter_transform_t* thunkCreate(
      void* ctx, PJ_string_view_t id, PJ_error_t* out_error) noexcept {
    auto& self = selfOf(ctx);
    if (id.data == nullptr || id.size == 0) {
  ...
  static void thunkListIds(
      void* ctx, PJ_string_view_t* out_ids, size_t capacity, size_t* out_count) noexcept {
    auto& self = selfOf(ctx);
