...
MessageParserLibrary::MessageParserLibrary(
    std::shared_ptr<void> handle, const PJ_message_parser_vtable_t* vtable, std::string path, std::string resolved_path,
...
Expected<MessageParserLibrary> MessageParserLibrary::loadFromHandleWithIdentity(
  ...
  auto sym = detail::resolveSymbol(handle.get(), "PJ_get_message_parser_vtable", recorded_path);
  if (!sym) {
