...
struct SourceDescriptorPolicy {
  ...
  /// Keys refused at ANY depth (exact, case-sensitive match) — the
  /// credential-smuggling guard. Empty (the default) checks nothing. A hit
  /// fails parseSourceDescriptor with kDeniedKeyViolationPrefix + key.
...
/// The error a denied key produces: this prefix followed by the offending
/// key, so callers can recognize the violation class and recover the key.
inline constexpr std::string_view kDeniedKeyViolationPrefix = "carries credential-shaped key: ";
...
/// object over max_container_entries, any policy.denied_keys key at any
/// depth, or nesting deeper than 16 levels.
