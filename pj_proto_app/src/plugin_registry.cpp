...
void PluginRegistry::scanDirectory() {
  ...
  for (const auto& entry : fs::recursive_directory_iterator(plugin_dir_)) {
    if (!entry.is_regular_file() || entry.path().extension() != PJ::PlatformUtils::pluginExtension()) {
      continue;
    }
...
void PluginRegistry::reload() {
  ...
  // Remove entries whose plugin file no longer exists on disk
  ...
  // Load new plugin files; reload modified ones
